...
<div class="fieldcontain ${hasErrors(bean: businessUserInstance, field: 'password', 'error')} required">
	...
	<g:textField name="password" required="" value="${businessUserInstance?.password}"/>
...
<div class="fieldcontain ${hasErrors(bean: businessUserInstance, field: 'gender', 'error')} required">
	...
	<g:select name="gender" from="${com.standconnect.security.SecUser$Gender?.values()}" keys="${com.standconnect.security.SecUser$Gender.values()*.name()}" required="" value="${businessUserInstance?.gender?.name()}" />
...
<div class="fieldcontain ${hasErrors(bean: businessUserInstance, field: 'businesses', 'error')} ">
	<label for="businesses">
		<g:message code="businessUser.businesses.label" default="Businesses" />
		
	</label>
	
<ul class="one-to-many">
<g:each in="${businessUserInstance?.businesses?}" var="b">
    <li><g:link controller="business" action="show" id="${b.id}">${b?.encodeAsHTML()}</g:link></li>
</g:each>
<li class="add">
<g:link controller="business" action="create" params="['businessUser.id': businessUserInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'business.label', default: 'Business')])}</g:link>
</li>
</ul>
